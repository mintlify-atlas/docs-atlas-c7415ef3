...
description: "How to migrate between OpenSpec versions and upgrade from legacy commands to OPSX commands."
---
...
The migration detects whichever tools you have configured and cleans up their legacy files. The removal list may seem long, but these are all files that OpenSpec originally created. Your own content is never deleted.

### What needs your attention

<Warning>
  **`openspec/project.md`** is not deleted automatically because it may contain project context you've written. You'll need to review it, move useful context to `openspec/config.yaml`, then delete the file when ready.
</Warning>

The old `project.md` was passive — agents might read it, might not, might forget what they read. The new `config.yaml` context is **actively injected into every OpenSpec planning request**, meaning your project conventions, tech stack, and rules are always present when the AI creates artifacts.

Because context is injected into every request, keep it concise. Focus on:
...
Both `openspec init` and `openspec update` detect legacy files and guide you through the same cleanup process.

<Note>
  New installs default to the `core` profile (`propose`, `explore`, `apply`, `archive`). Migrated installs preserve your previously installed workflows by writing a `custom` profile when needed.
</Note>
...
<Steps>
  <Step title="Remove legacy slash commands">
    Legacy slash command directories are removed from your tool directories.
  </Step>
  <Step title="Strip OpenSpec markers">
    OpenSpec markers are stripped from `CLAUDE.md`, `AGENTS.md`, etc. Your content stays intact.
  </Step>
  <Step title="Delete openspec/AGENTS.md">
    The obsolete `openspec/AGENTS.md` file is deleted.
  </Step>
  <Step title="Install new skills">
    New skills are installed in `.claude/skills/`.
  </Step>
  <Step title="Create config.yaml">
    `openspec/config.yaml` is created with a default schema.
  </Step>
</Steps>
...
For scripted migrations, use the `--force` flag to skip prompts and auto-accept cleanup:
...
<Steps>
  <Step title="Create config.yaml">
    If not already created by init:
  ...
  <Step title="Add per-artifact rules (optional)">
    ```yaml
    rules:
      proposal:
...
<Tip>
  Don't overthink it. Start with the essentials and iterate. If you notice the AI missing something important, add it. If context feels bloated, trim it.
</Tip>
...
<Warning>
  The legacy `/openspec:proposal`, `/openspec:apply`, and `/openspec:archive` commands no longer exist as of 1.0.0. Use the equivalents below.
</Warning>
...
When determining which schema to use, OPSX checks in this order:
...
<AccordionGroup>
  ...
  <Accordion title="Want to see what would be cleaned up?">
